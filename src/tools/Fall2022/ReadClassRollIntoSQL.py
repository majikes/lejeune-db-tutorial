...
args = Args.Parse(csv='classroll.COMP421.csv')
...
roll = roll.fillna('')
print(len(roll.loc[roll.section == 0]), 'in section 0')
                ...
                (onyen, pid, name, section, imageURL, zwsp) =
                      (%(onyen)s, %(pid)s, %(name)s, %(section)s, %(imageURL)s, %(zwsp)s)''',
                        {'onyen': onyen, 'pid': pid, 'name': name,
                         ...
                         'zwsp': zwsp})
         # Since the above insert may change the rid which will change the zwsp
         cursor.execute('''SELECT rid, zwsp FROM roll WHERE onyen=%(onyen)s''', dict(onyen=onyen))
         row = cursor.fetchone()
         if row.rid != rid or row.zwsp != zwsp:
            zwsp = zwsp_object.getSequence(row.rid)
            cursor.execute('''UPDATE roll SET zwsp=%(zwsp)s WHERE onyen=%(onyen)s''',
                           dict(onyen=onyen, zwsp=zwsp))
         connection.commit()
