...
def getInfoWithExceptions(info, key, onyen=None, section=None, default=None):
      """ Return the info value for the key """
      exceptions = info.get("exceptions", {})
      orderd_info = {**info,
                     **exceptions.get(f"_{section}", {}),
                     **exceptions.get(onyen, {})}
      return orderd_info.get(key, default)
...
class inputs:
    ...
    def _name(self, n):
        ...
        return (n.strip() or "Q{}").format(self.number)
    ...
    def expression(self, answer, *contexts, correction_rate=5, points=0, name="", **attrs):
        ...
        name = self._name(name)
        assert len(contexts) > 0, f' {self.key} question {name} has no tests!'
    ...
    def javascriptProgram(self, answer, *contexts, correction_rate=10,
                          points=0, name="", **attrs):
        ...
        name = self._name(name)
        tests = [{"context": context} for context in contexts]
        for inval in ['num_rows', 'placeholder']:
           assert inval not in kwargs, f'{self.key}:{name}: {inval} cannot be specified as it is not used in code mirror'
        self._addRubric(name, answer, "function", points, correction_rate, tests=tests, **attrs)
        return template("javascriptProgram", name=name,
    ...
    def table(self, inputs=0, headings="", table="", symbols="",
        ...
        """Display a table
        inputs are the number of columns passed in that are input to the test
        In other words, the remaining columns after input are answers
        """
        name = self._name(name)
    ...
    def sql(self, answer, points=0, name="", dbs=[], sort=False, timeout=2000, correction_rate=-1, ics=[], **kwargs):
        """A SQL query"""
        name = self._name(name)
        if name[0] != '_':
           # If it's a countable SQL, check for sort and order by agreement 
           if bool(sort) and re.search(r'order\s+by', answer, re.IGNORECASE):
               print(f'Warning: For {self.key}.{name} sort={sort} and ORDER BY')
           if not bool(sort) and not re.search(r'order\s+by', answer, re.IGNORECASE):
               print(f'Warning: For {self.key}.{name} sort=False and ORDER BY specified')
        static = bottle.SimpleTemplate.defaults["static"]
        tests = [dict(db=static(osp.join("dbs", name)),
                      ic=0) for name in dbs]
        tests += [dict(db=static(osp.join("dbs", name)),
                       ic=ic) for name in dbs for ic in ics]
        for inval in ['num_rows', 'placeholder']:
           assert inval not in kwargs, f'{self.key}:{name}: {inval} cannot be specified as it is not used in code mirror'
        self._addRubric(
            name, answer, "sql", points, tests=tests, sort=sort, timeout=timeout, correction_rate=correction_rate,
        )
        return template("sql", name=name, points=points, **kwargs)
    ...
    def setInfo(self, **values):
        ...
        for deprecated in ['due', 'pages', 'needSubmitCode', 'needsSubmitCode', 'maxpenalty', 'penalty']:
            assert deprecated not in values, f'{self.key}: Use of setInfo of {deprecated} is no longer supported'
