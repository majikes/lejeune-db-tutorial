...
DATABASES = ['midterm2-nc-legislature-1.sqlite', 'midterm2-nc-legislature-2.sqlite']
    ...
    with closing(conn.cursor()) as cursor:
        ...
        cursor.execute(f'''
           CREATE TABLE Votes
               (roll_call_id INTEGER REFERENCES Roll_calls(Roll_call_id),
                legislator_id INTEGER REFERENCES Legislators(legislator_id),
        ...
        cursor.execute('''
           CREATE TABLE Bill_subjects
               (subject_id INTEGER REFERENCES Subject_names(subject_id),
                bill_id INTEGER REFERENCES Bills(bill_id),
...
with closing(conns[1].cursor()) as cursor:
    ...
    for party in ['Democratic', 'Republican']:
        for role in ['Representative', 'Senator']:
            cursor.execute(f'''
               WITH Bill_counts AS (SELECT COUNT(DISTINCT L.legislator_id) AS bill_count, B.bill_id
                                      FROM Sponsors S, Legislators L, Bills B
                                     WHERE S.bill_id = B.bill_id AND
                                           S.legislator_id = L.legislator_id AND
                                           L.party = '{party}' AND
                                           L.role = '{role}'
                                     GROUP BY B.bill_id),
                       largest_bill AS (SELECT MAX(bill_count) as bill_count, bill_id
                                          FROM Bill_counts
                                         GROUP BY bill_id
                                         ORDER BY MAX(bill_count) DESC
                                         LIMIT 1),
                       other_than_largest AS (SELECT MAX(B.bill_count) as bill_count, B.bill_id
                                                FROM Bill_Counts B, largest_bill L
                                               WHERE B.bill_count != L.bill_count
                                               GROUP BY B.bill_id
                                               ORDER BY MAX(B.bill_count) DESC
                                               LIMIT 1)

                  SELECT bill_count, bill_id
                    FROM other_than_largest
                  UNION
                  SELECT bill_count, bill_id
                    FROM largest_bill''')
            penultimate_info = cursor.fetchone()
            max_info = cursor.fetchone()
            num_to_add = max_info['bill_count'] - penultimate_info['bill_count']
            cursor.execute(f'''
                  SELECT L.legislator_id
                    FROM Legislators L
                   WHERE L.party = '{party}' AND
                         L.role = '{role}' AND
                         L.legislator_id not in (SELECT S.legislator_id
                                                   FROM Sponsors S
                                                  WHERE bill_id = {penultimate_info['bill_id']})
                   LIMIT {num_to_add}''')
            rows = cursor.fetchall()
            for row in rows:
                cursor.execute(f'''
                      INSERT INTO Sponsors (bill_id, Legislator_id)
                                    VALUES({penultimate_info['bill_id']},
                                           {row['legislator_id']})''')
    conn.commit()
