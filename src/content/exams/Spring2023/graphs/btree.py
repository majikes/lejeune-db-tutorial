...
class BPlusTree:
    ...
    def __init__(self, keys_per_block, tree, cellborder):
...
def generate_dot_node(tree, index, block, cellborder):
...
def generate_cross_edge_range(tree, leaves, two_head=False):
...
def generate_cross_edges(tree):
    ...
    while True:
        ...
        yield from generate_cross_edge_range(tree, adjacent_leaves, two_head=True)
...
def generate_dot_graph(tree):
    ...
    nodes = '\n'.join(generate_dot_node(tree, i, k, tree.cellborder) for i, k in tree.all_blocks)
...
def main():
    ...
    tree = BPlusTree(data['keys_per_block'], data['tree'], data.get('cellborder', 0))
