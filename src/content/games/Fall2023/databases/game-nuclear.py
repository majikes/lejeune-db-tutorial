...
incident_dates = [first_day_of_classes + timedelta(days=3*x) for x in range(NUM_DBS)]
...
def create_tables(conn):
    ...
    with closing(conn.cursor()) as cursor:
        ...
        cursor.execute('''CREATE TABLE Drivers_licenses(
                                        drivers_license_id INTEGER PRIMARY KEY,
                                        person_id INTEGER,
        ...
        cursor.execute('''CREATE TABLE Parts(
                                        ...
                                        installed_by_person_id INTEGER DEFAULT Null,
                                        CHECK (installed IN (0, 1)),
                                        FOREIGN KEY(installed_by_person_id)
                                           REFERENCES Persons(person_id)
                                           ON DELETE SET NULL,
                                        FOREIGN KEY(company_id)
                                           REFERENCES Companies(company_id)
        ...
        cursor.execute('''CREATE TABLE Badge_Readers(
                                        badge_reader_id INTEGER PRIMARY KEY,
                                        from_room INTEGER,
                                        to_room INTEGER,
...
def insert_random_person(cursor, index, report_info, street_name=None, address_number=None,
    ...
    if index % 5 == 1:
        ...
        cursor.execute(f'''INSERT INTO Badges(person_id, expired, rfid)
                                  VALUES ({person_id}, 1, '{faker.uuid4()}')''')
    cursor.execute(f'''INSERT INTO Badges(person_id, expired, rfid)
                              VALUES ({person_id}, 0, '{faker.uuid4()}')''')
    badge_id = cursor.lastrowid

    # Insert at least one door opening before the incident
    cursor.execute('SELECT badge_reader_id FROM Badge_Readers')
    badge_reader_ids = [x['badge_reader_id'] for x in cursor.fetchall()]
    cursor.execute(f'''INSERT INTO Doors_Opened (badge_id, badge_reader_id, open_time)
    ...
    if index % 10 == 0:
        ...
        cursor.execute(f'''INSERT INTO Drivers_licenses(person_id, age, height_inches, eye_color, hair_color,
                                                       gender, plate_number, car_make, car_model, expired)
                                  VALUES ({person_id}, {age}, {height}, '{eye_color}', '{hair_color}',
...
def create_contact(cursor, index, report_info):
    ...
    if index % 2 == 0:
        ...
        cursor.execute(f'''DELETE FROM Drivers_licenses
                                WHERE eye_color='{contact_eye_color}' AND
                                      hair_color='{contact_hair_color}' AND
                                      gender='{contact_gender}' ''')
        person_id = insert_random_person(cursor, index, report_info, eye_color=contact_eye_color,
        ...
        # Ensure uniqueness
        cursor.execute(f'''UPDATE Drivers_licenses SET expired=1
                                  WHERE eye_color='{contact_eye_color}' AND
                                        hair_color='{contact_hair_color}' AND
...
You need to trust me.'''
        ...
        person_id = insert_random_person(cursor, index, report_info,
...
with license plate {contact_plate_number}.
...
You need to trust me.'''
    ...
    assert len(rows) == 1, f'''Don't know how, but there are {len(rows)} rows in Persons table with person_id={person_id}'''
    ...
    cursor.execute(f'''INSERT INTO Drivers_licenses (person_id,
                         expired, age, height_inches, eye_color, hair_color,
                         gender, plate_number, car_make, car_model)
                                   SELECT {int(contact_info['person_id'])+1}, 1,
                                          age, height_inches, eye_color, hair_color,
                                          gender, plate_number, car_make, car_model
                                     FROM Drivers_licenses
                                    WHERE person_id='{contact_info['person_id']}' ''')
    return contact_info

def insert_random_report(cursor, index, incident_type=None, incident_date=None, incident_description=None, person_id=None):
...
'''Did you hear that Professor Majikes blather on and on and on?''']
    ...
    if not incident_date:
        incident_date =  first_day_of_classes + [1,-1][index%2] * timedelta(days=4*index)
    if not incident_description:
        incident_description = random.choice(descriptions)
    if not person_id:
        cursor.execute('''SELECT person_id FROM Persons''')
        person_id = random.choice([x['person_id'] for x in cursor.fetchall()])

    cursor.execute(f'''INSERT INTO Incident_Reports (incident_date, incident_type, description, person_id)
                            VALUES ('{sqlite_date(incident_date)}',
                                    '{incident_type}', '{incident_description}', {person_id}) ''')
...
I am going to leave the scientist in the control room as I go investigate some noise outside.'''
    ...
    insert_random_report(cursor, index, incident_date=report_info['incident_date'],
                         incident_description=incident_description, person_id=contact_info['person_id'])

    # Find someone from SQL City Nuclear Power Company besides contact_person to issue the gunfire incident
    if contact_info['contact_type'] == 'color':
        cursor.execute(f'''SELECT P.person_id
                             FROM Persons P, Companies C, Drivers_licenses D
                            WHERE P.company_id = C.company_id AND
                                  P.person_id = D.person_id AND
                                  P.person_id != {contact_info['person_id']} AND
                                  (D.eye_color != '{contact_info["color"]["eye_color"]}' or
                                   D.hair_color != '{contact_info["color"]["hair_color"]}' or
                                   D.gender != '{contact_info["color"]["gender"]}')
                            ORDER BY P.person_id {'DESC' if member % 2 == 0 else 'ASC'}''')
    else:
        cursor.execute(f'''SELECT P.person_id
                             FROM Persons P, Companies C, Drivers_licenses D
                            WHERE P.company_id = C.company_id AND
                                  P.person_id = D.person_id AND
                                  P.person_id != {contact_info['person_id']} AND
                                  (D.car_make != '{contact_info["car"]["car_make"]}' or
                                   D.car_model != '{contact_info["car"]["car_model"]}' or
                                   D.plate_number != '{contact_info["car"]["plate_number"]}')
                            ORDER BY P.person_id {'DESC' if member % 2 == 0 else 'ASC'}''')
    reporter_person_id = cursor.fetchall()[0]['person_id']
    description = 'Shots fire! Shots fire! Loud explosion'
    insert_random_report(cursor, index, incident_type='Code Red', incident_date=report_info['incident_date'],
                         incident_description=description, person_id=reporter_person_id)
...
def insert_companies(cursor, index):
    ...
    POWER_COMPANY['company_id'] = (index+1)*NUM_DBS
    cursor.execute(f'''INSERT INTO Companies
                                        (company_id, name, address_number,
                                         address_street_name, phone_number)
                                   VALUES({POWER_COMPANY['company_id']}, '{POWER_COMPANY["name"]}',
    ...
    POWER_COMPANY['all_ids'] = [x['company_id'] for x in cursor.fetchall()]
...
def create_rooms(cursor):
    ...
    for i, room in enumerate(ROOM_NAMES):
        cursor.execute(f'''INSERT INTO Rooms (room_id, name, phone_number)
...
def create_saboteur_info(cursor, index, company_info, report_info):
    ...
    cursor.connection.row_factory = sqlite3.Row
    cursor.execute(f'''SELECT P.person_id, P.name, P.employee_type, B.badge_id, DL.gender
                         FROM Persons AS P, Companies as C, Badges as B, Drivers_licenses as DL
                        WHERE C.name='{company_name}' AND
                              P.company_id=C.company_id AND
                              DL.person_id =B.person_id AND
                              P.person_id=B.person_id AND
                              P.employee_type != 'permanent-employee' ''')
    rows = cursor.fetchall()
    row = rows[ len(rows) // 2 ]
    saboteur_info['person_id'] = row['person_id']
    ...
    cursor.execute(f'''SELECT R.name, R.phone_number, R.room_id, BR.badge_reader_id
                         FROM Rooms as R, Badge_Readers as BR
                        WHERE R.name in {tuple(IMPORTANT_ROOM_NAMES)} AND
                              R.room_id = BR.to_room ''')
    ROOM_INFO = namedtuple('ROOM_INFO', 'room_id, phone_number badge_reader_id')
    rooms_info = {}
    for row in cursor.fetchall():
        if row['name'] not in rooms_info:
            rooms_info[row['name']] = ROOM_INFO(room_id=row['room_id'],
    ...
    cursor.execute(f'''UPDATE Parts
                          SET installed=true
                        WHERE installed_by_person_id={saboteur_info['person_id']}''')
    ...
    for team in range(NUM_TEAMS+1):
        for member in range(NUM_MEMBERS):
            ...
            with closing(conn.cursor()) as cursor:
                ...
                if contact_info['contact_type'] == 'color':
                    cursor.execute(f'''SELECT P.phone_number, I.incident_date
                                         FROM Drivers_licenses AS D, Persons AS P, Incident_Reports AS I
                ...
                assert rows[0][0] == contact_info['phone_number'], f"Contact information has person_id {rows[0][0]} for: {database_fn} {contact_info}"
            ...
            with closing(conn.cursor()) as cursor:
                ...
                if contact_info['contact_type'] == 'color':
                    cursor.execute(f'''SELECT DISTINCT P.phone_number
                                         FROM Drivers_licenses as D, Persons AS P
            ...
            with closing(conn.cursor()) as cursor:
                ...
                cursor.execute(f'''SELECT count(*)
                                     FROM Parts, Persons
                                    WHERE Persons.person_id={saboteur_info['person_id']} AND
                                          Persons.person_id=Parts.installed_by_person_id AND
                                          installed=true''')
                rows = cursor.fetchall()
                assert len(rows) == 1, f"Database {database_fn} saboteur_info has person_id {saboteur_info['person_id']} should have one part INSTALLED and it has {len(rows)}"
