...
NUM_DBS = (NUM_TEAMS + 1) * NUM_MEMBERS
# If you change the NUM_DBS, the accused, professor, advocate will all changea
...
def python_date(this_date):
    ''' Given a string Return the date object '''
    return date(int(this_date[0:4]),
                int(this_date[5:7]),
                int(this_date[8:10]))
...
def create_tables(conn):
    ...
    with closing(getCursor(conn)) as cursor:
        ...
        cursor.execute(f"""create table section
                                     (course_id    varchar(8),
                                      sec_id       varchar(8),
                                      semester     varchar(6)
                                         check (semester in ('Fall', 'Winter', 'Spring', 'Summer')),
                                      year         numeric(4,0) check (year > 1701 and year <= {THIS_YEAR}),
                                      building     varchar(15),
                                      room_number  varchar(7),
        ...
        cursor.execute("""create table conversation_participants
                                     (transcription_id  integer references transcription(transcription_id),
                                      id                varchar(8),
...
def get_accused_student(conn, index, gd):
    ...
    with closing(getCursor(conn)) as cursor:
        ...
        choice_index = 1
        last_name = last_names[-(index+choice_index) % len(last_names)]
        while last_name == gd['student_advocate']['last_name']:
            choice_index += 1
            last_name = last_names[-(index+choice_index) % len(last_names)]
...
def get_course_with_accused_advocate(conn, gd, index):
    ...
    with closing(getCursor(conn)) as cursor:
        ...
        if THIS_SEMESTER == 'Spring':
            course['conversation_date'] = f'{THIS_YEAR}-04-28'
        else:
            course['conversation_date'] = f'{THIS_YEAR}-11-30'
        course['conversation_time'] = f'09:{(index+12)%60}'
        course['cheating_date'] = sqlite_date(dates_of_cheating[index % len(dates_of_cheating)])
        course['conversation_text'] = [conversation]
        ...
        conversation = f'''I am sorry {gd['accused_student']['first_name']}, but the
video clearly shows a {'man' if gd['accused_student']['gender'] == 'male' else 'woman'} with
{gd['accused_student']['hair_color']} hair and {gd['accused_student']['eye_color']} eyes, you,
talking to several students for several minutes during the exam on {python_date(course['cheating_date']).strftime('%b %d')}.'''
        course['conversation_text'].append(conversation)
        insert_conversation(cursor, course['conversation_date'],
                            course['conversation_time'], conversation,
                            [course['teacher_id'], gd['accused_student']['id']])
...
def insert_with_error_msg(cursor, query, query_dict, print_dict=True):
    '''Insert using query/vars but if it fails, issue a usable message'''
    try:
...
def populate_course_with_students(conn, gd, number_students):
    ''' Given a course (couose_id, sec_id, semester, year), ensure there are number_students taking it'''
    sql_parms = dict(course_id=gd['course']['course_id'],
                     sec_id=gd['course']['sec_id'],
                     semester=gd['course']['semester'],
                     time_slot_id=gd['course']['time_slot_id'],
                     eye_color=gd['accused_student']['eye_color'],
                     hair_color=gd['accused_student']['hair_color'],
                     gender=gd['accused_student']['gender'],
                     year=gd['course']['year'])
    with closing(getCursor(conn)) as cursor:
        ...
        assert len(student_ids) < number_students
        # Find available students who are not of the same hair color, etc to add to the class
        cursor.execute(f''' SELECT S.id
                             FROM Student S, Id_Card I
                            WHERE S.ID NOT IN {tuple(student_ids)} AND
                                  S.id=I.id AND
                                  (I.eye_color != :eye_color OR
                                   I.hair_color != :hair_color OR
                                   I.gender != :gender)
                           ''',
                       sql_parms)
        available_ids = [x[0] for x in cursor.fetchall()]
        new_ids = random.sample(available_ids, number_students-len(student_ids))
        for s_id in new_ids:
            sql_parms['id'] = s_id
            insert_with_error_msg(cursor,
                                  """ INSERT INTO Takes (id, course_id, sec_id, semester, year)
                                                 VALUES (:id, :course_id, :sec_id, :semester, :year)""",
                                  sql_parms)

        # Add some bogus Sections / Takes
        cursor.execute('''
              SELECT building, room_number
                FROM classroom''')
        classrooms = [(x[0], x[1]) for x in cursor.fetchall()]
        building, room_number = random.choice(classrooms)
        sql_parms['building'] = building
        sql_parms['room_number'] = room_number

        # Different semester id
        sql_parms_copy = sql_parms.copy()
        sql_parms_copy['sec_id'] = sql_parms['sec_id'] + 3
        insert_with_error_msg(cursor,
                              """ INSERT INTO Section (course_id, sec_id, semester, year, building, room_number, time_slot_id)
                                               VALUES (:course_id, :sec_id, :semester, :year, :building, :room_number, :time_slot_id)""",
                              sql_parms_copy)
        sql_parms_copy['id'] = random.choice(available_ids)
        insert_with_error_msg(cursor,
                              """INSERT INTO Takes (id, course_id, sec_id, semester, year)
                                            VALUES (:id, :course_id, :sec_id, :semester, :year)""",
                              sql_parms_copy)
        ...
        # Different year
        sql_parms_copy = sql_parms.copy()
        sql_parms_copy['year'] = sql_parms['year'] - 1
        insert_with_error_msg(cursor,
                              """ INSERT INTO Section (course_id, sec_id, semester, year, building, room_number, time_slot_id)
                                               VALUES (:course_id, :sec_id, :semester, :year, :building, :room_number, :time_slot_id)""",
                              sql_parms_copy)
        ...
        # Different semester (try different ones as there are some random summer, spring semesters)
        sql_parms_copy = sql_parms.copy()
        for semester in SEMESTERS:
            sql_parms_copy['semester'] =  semester
            try:
                insert_with_error_msg(cursor,
                                  """ INSERT INTO Section (course_id, sec_id, semester, year, building, room_number, time_slot_id)
                                                   VALUES (:course_id, :sec_id, :semester, :year, :building, :room_number, :time_slot_id)""",
                                  sql_parms_copy, print_dict=False)
            except sqlite3.IntegrityError:
                sql_parms_copy['semester'] = None
                continue
            break
        sql_parms_copy['id'] = random.choice(available_ids)
        insert_with_error_msg(cursor,
        ...
        # Make one student in the class to have a expired id card
        cursor.execute(f"""
              SELECT i.ID, i.name, i.DOB, i.height_inches, i.ssn, s.dept_name, s.tot_cred
                FROM ID_Card i, student s
               WHERE i.expired=false AND
                     S.id=i.id AND
                     S.ID IN {tuple(new_ids)} AND
                     gender = '{'male' if gd['accused_student']['gender'] == 'female' else 'female'}'  """)
        expired_id, expired_name, DOB, height_inches, ssn, dept_name, tot_cred = cursor.fetchone()
        expired_name = 'Eden ' + expired_name.split()[-1]

        bogus_parms = dict(id='ex-'+expired_id,
                           name=expired_name,
                           DOB=DOB,
                           ...
                           sec_id=gd['course']['sec_id'],
                           semester=gd['course']['semester'],
                           course_id=gd['course']['course_id'],
                           year=gd['course']['year'],
                           expired=True)
        cursor.execute("""
              INSERT INTO id_card (ID, name, DOB, height_inches, ssn,
                                   eye_color, hair_color, gender, expired)
                            VALUES (:id, :name, :DOB, :height_inches, :ssn,
                                    :eye_color, :hair_color, :gender, :expired)  """,
                       bogus_parms)
        cursor.execute("""
              INSERT INTO Student (ID, dept_name, tot_cred)
                            VALUES (:id, :dept_name, :tot_cred)  """,
                       bogus_parms)
        cursor.execute("""
...
for team in range(NUM_TEAMS+1):
    for member in range(NUM_MEMBERS):
        ...
        print(f'{datetime.now().strftime("%H:%M:%S")} Creating database {gameData[game_index]["database"]} number {index+1} of {NUM_DBS}')
        ...
        gameData[game_index]['course'] = get_course_with_accused_advocate(conn, gameData[game_index], index)
        populate_course_with_students(conn, gameData[game_index], 30)
